...
install: |
  ...
  # NOT tied to Google Antigravity — plain agent-skill format, copied into Codex's
  # skill dir. See claude-code/science-skills for the full overview.
  #
  ...
  mkdir -p /tmp/sci "$HOME/.codex/skills"
  cd /tmp/sci
  git init -q
  ...
  # Copy each self-contained skill dir into Codex's skill dir as a set
...
# Science Skills (Google DeepMind) — Codex

The Google DeepMind [Science Skills](https://github.com/google-deepmind/science-skills)
bundle for the Codex harness — copied into `~/.codex/skills/`. See
...
> Codex's skill discovery is less battle-tested than Claude Code's. The skills
> install into `~/.codex/skills/` (the same path ECC's harness-aware installer
> uses); if a future Codex version expects a different location, adjust the copy
> target here.
...
- **Optional API keys** — most need none; **AlphaGenome** and **OpenAlex** require
  a key, **ClinVar** benefits from one. Provide as env vars.
