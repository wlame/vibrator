...
`)
	b.WriteString("# Shell completions for the always-on CLI tools (rg, just) — generated by\n")
	b.WriteString("# the tools themselves so they always match the installed version. All\n")
	b.WriteString("# three supported shells get files; a missing shell just ignores its dir.\n")
	b.WriteString("# No `|| true`: both tools were installed and version-gated above, so a\n")
	b.WriteString("# generation failure is a real regression that must fail the build.\n")
	b.WriteString("RUN mkdir -p /usr/local/share/zsh/site-functions /usr/share/bash-completion/completions /usr/share/fish/vendor_completions.d && \\\n")
	b.WriteString("    rg --generate complete-zsh  > /usr/local/share/zsh/site-functions/_rg && \\\n")
	b.WriteString("    rg --generate complete-bash > /usr/share/bash-completion/completions/rg && \\\n")
	b.WriteString("    rg --generate complete-fish > /usr/share/fish/vendor_completions.d/rg.fish && \\\n")
	b.WriteString("    just --completions zsh  > /usr/local/share/zsh/site-functions/_just && \\\n")
	b.WriteString("    just --completions bash > /usr/share/bash-completion/completions/just && \\\n")
	b.WriteString("    just --completions fish > /usr/share/fish/vendor_completions.d/just.fish\n")
	b.WriteString("\n")
	b.WriteString(`# fd-find via apt (Ubuntu names the binary fdfind; we symlink to fd).
