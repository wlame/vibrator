...
# Shell completions for the always-on CLI tools (rg, just) — generated by
# the tools themselves so they always match the installed version. All
# three supported shells get files; a missing shell just ignores its dir.
# No `|| true`: both tools were installed and version-gated above, so a
# generation failure is a real regression that must fail the build.
RUN mkdir -p /usr/local/share/zsh/site-functions /usr/share/bash-completion/completions /usr/share/fish/vendor_completions.d && \
