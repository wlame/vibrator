...
#   vibrate build-dockerfile --harness=opencode --profile=frontend --shell=bash
...
# --- feature: playwright (Playwright + Chromium) ---
RUN apt-get update && apt-get install -y --no-install-recommends \
...
ENV VIBRATOR_HARNESS="opencode"
ENV VIBRATOR_PROFILE="frontend"
ENV VIBRATOR_FEATURES_LIST="node,playwright"
...
LABEL vibrator.harness="opencode"
LABEL vibrator.profile="frontend"
LABEL vibrator.shell="bash"
LABEL vibrator.features="node,playwright"
LABEL vibrator.extensions="(none)"


ENTRYPOINT ["/opt/vibrator/entrypoint.sh"]
CMD ["/usr/local/bin/claude-exec", "/bin/bash"]
